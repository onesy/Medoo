...
class medoo {
    ...
    public function __construct($host, $username, $password, $port = 3306, $database_name = '', $charset = 'utf8', $options = null) {
        ...
        try {
            ...
            switch ($type) {
                ...
                case 'pgsql':
                    $this->pdo = new \PDO(
                            $type . ':host=' . $this->server . ';' . $port . 'dbname=' . $this->database_name, $this->username, $this->password, $this->option
                    );
                    break;
                ...
                case 'sqlite':
                    $this->pdo = new \PDO(
                            $type . ':' . $this->database_file, $this->option
                    );
                    break;
            }
    ...
    // check the pdo link is usefull or not
    public function check() {
        $query = "select 1+1";
        if (!$rtn = $this->pdo->query($query)) {
            $this->pdo = $this->reset_pdo();
        }
    }
